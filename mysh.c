    ...
    for (proc = job->root; proc != NULL; proc = proc->next) {
        ...
        if (proc->next != NULL) {
            ...
            if (proc->output_path != NULL) {
                ...
                else if (proc->output_type == TWO_SHIFT)
                {
...
struct process* ft_parse_command_segment(char *segment) {
    ...
    char *token;
    char **tokens = (char**) malloc((bufsize + 1) * sizeof(char*));
    tokens[bufsize] = NULL;
    bool    is_valid = true;
    if (!tokens) {
    ...
    new_proc->next = NULL;
    new_proc->valid = is_valid;
    new_proc->output_type = out_type;
    return new_proc;
}
